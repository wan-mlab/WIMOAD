...
def train_meta_model_and_predict(idx, meta_name, meta_model, 
    ...
    for i in test_idx:
        results.append({
            ...
            "Combination": comb,
            "MetaModel": meta_name,
            "SampleRID": X_test.index[0],
            "Prediction": meta_predictions[i, idx],
            "Prediction_f": meta_predictions2[i, idx]
        })

# def run_single_loo_parallel(repeat_idx, data, labels, cv_method, n_splits, classifiers, meta_models):
...
def run_single_loo_parallel(repeat_idx, data, labels, cv_method, n_splits, classifiers, meta_models):
    ...
    # Parallelize the outer loop (LOO iterations)
    parallel_results = Parallel(n_jobs=-1)(
        delayed(process_loo_iteration)(
            repeat_idx, data, labels, outer_cv, train_idx, test_idx,
            classifiers, meta_models, combinations, 
            base_predictions_out, meta_predictions, meta_predictions2
        )
        for train_idx, test_idx in outer_cv.split(data, labels)  # Iterate over LOO splits
    )
...
def process_loo_iteration(repeat_idx, data, labels, outer_cv, train_idx, test_idx, 
                           classifiers, meta_models, combinations, 
                           base_predictions_out, meta_predictions, meta_predictions2):
    print(f"Processing Test: {test_idx}")
    # Extract the train and test splits directly from train_idx and test_idx
    X_train_o, X_test_o = data.iloc[train_idx], data.iloc[test_idx]
    y_train, y_test = labels.iloc[train_idx], labels.iloc[test_idx]
    ...
    for comb in combinations:
        ...
        if len(comb) == 1:
            ...
            parallel_train_meta_models(
                meta_models, meta_train_X, y_train, meta_test_X, 
                X_train, X_test, test_idx, comb, 
                meta_predictions, meta_predictions2, iteration_results, data, extended_meta_train_X, extended_meta_test_X, repeat_idx
            )
